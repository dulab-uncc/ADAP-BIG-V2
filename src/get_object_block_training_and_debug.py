...
from mzml_reader import extract_mzvals, extract_timevals, extract_intensities, netCDFReadHandler
...
def extract_training_data(profile_file_mzML, window_mz=60, window_rt=300):
    dataFramePrevEvaluation = pd.read_csv(r"C:\Users\jerry\Desktop\DCSM.csv")
    dataFrameV2Prediction = pd.read_csv(params.results_path + "\ADAP-3D-Predictions.csv")
    confirmedpeaks = pd.read_csv(r"C:\Users\jerry\Desktop\files.filtered.DCSM exclusive.csv")
    mzvals = list(map(float, confirmedpeaks['m/z']))
    timevals = confirmedpeaks['rt']

    dataFramePrevEvaluation2 = pd.read_csv(r"C:\Users\jerry\Desktop\files.filtered.part6 (1).csv")
    ...
    if(".cdf" in profile_file_mzML):
        ## Scanning the raw .cdf file into array data ##
        file = netCDFReadHandler(profile_file_mzML)
        tics = file.get_TIC()
        scan_t = list(tics['scan_acquisition_time'])
        mz_list = []
        intensity_list = []
        for i in range(len(scan_t)):
            temp = file.get_one_scan_by_scan_index(i)
            mz_list.append(list(temp['mz']))
            intensity_list.append(list(temp['intensity']))
    else:
    ...
    for time in range(143, 200):
        ...
        if pos_mz2orig >= len(newallmzintime):
        ...
        for t in range(pos_rt1, pos_rt2):
            ...
            mzgrid = mz_list[t]
            mzleft = mzgrid.index(find_nearest(mzgrid, newallmzintime[pos_mz1orig]))
            mzright = mzgrid.index(find_nearest(mzgrid, newallmzintime[pos_mz2orig]))
            ...
            for i in range(window_mz * 2):
                ...
                if(len(temparr) == 0 ):
                    if(j>1 and j < len(binnedvals2)-1 and temparr[i- 1] != 0 and temparr[i+ 1] !=0):
                        finalbins.append((temparr[i - 1] + temparr[i + 1])/2)
                    else:
                        finalbins.append(0)
                else:
                    finalbins.append(max(temparr))

            for i in range(1, len(finalbins)-1):
                if(finalbins[i] == 0 and finalbins[i-1] != 0 and finalbins[i+1] != 0):
                    finalbins[i] = (finalbins[i-1] + finalbins[i+1])/2
        ...
        plt.margins(0, 0)
        plt.savefig(r'.\Filtered6\ ' + "filtered6 # " + str(cnt) + '.png')
        plt.close('all')
        #breakpoint()
