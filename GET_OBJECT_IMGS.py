    ...
    for time in range(len(predictionsArrTime)):
        ...
        if(float(predictionsArrTime[time]) < 20.0 and float(predictionsArrTime[time]) > 2.5):
            ...
            for xtick in range(2*window_mz):
                xticks.append(xtick)
            for xlabel in range(window_mz, -1, -1):
                xlabels.append(round(mz_list[pos_rt][pos_mzorig-xlabel], 6))
            for xlabel in range(1, window_mz):
                xlabels.append(round(mz_list[pos_rt][pos_mzorig+xlabel], 6))
            ...
            fig.colorbar(img)
            plt.savefig(RESULTS_PATH + '\Signal_Images\DCSM_IMGS\ ' + str(cnt) + '.png')
            plt.close()
        #breakpoint()
